...
class Test_Prob2:
    ...
    def test_prints_something(self, capsys):
        Prob2.divisible_by_six_or_seven()
    ...
    def test_within_thresholds(self):
        Prob2.list_divisible_by_six_or_seven(6,60)
        captured = capsys.readouterr().out.rstrip()
        values = [int(n) for n in captured.split('\n')]
        for num in values:
    ...
    def test_returns_correct_count(self):
        ...
        for r,c in zip(ranges, counts):
            ...
            assert sol == c, f"For a range from {r[0]} to {r[1]}, your function should have printed out {c} numbers and returned that number. But it is instead returning {sol}."
